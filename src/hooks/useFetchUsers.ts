...
import { transformUserData, UserDataBackend, UserDataFrontend } from '../adapter';
...
const useFetchUsers = (updateListFlag: boolean) => {
  ...
  const handleSearch = (searchTerm: string) => {
    ...
    setSearchTerm(term);
    const filtered = usersList.filter((user) => user.name.toLowerCase().includes(term));
    setFilteredUsers(filtered);
  };

  return { usersList,setUsersList, filteredUsers, setFilteredUsers, handleSearch };
};
