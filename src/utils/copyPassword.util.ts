export const copyPassword = (password: string) => {
	...
	if (navigator.clipboard) {
		navigator.clipboard
			...
			.catch(err => {
				console.error(
					'Error al copiar la contraseña usando navigator.clipboard: ',
					err,
				);
...
const fallbackCopy = (password: string) => {
	...
	try {
		...
		if (successful) {
			...
			console.error(
				'No se pudo copiar la contraseña usando document.execCommand.',
			);
		}
	} catch (err) {
		console.error(
			'Error al copiar la contraseña usando document.execCommand: ',
			err,
		);
	} finally {
