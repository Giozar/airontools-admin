...
function RoleList({updateRole}:{updateRole? : boolean}){
    ...
    const handleDelete = async ({roleName,roleId}:{roleName:string,roleId:string}) => {
        try {
          const response = await axios.delete(`http://localhost:4000/roles/delete/${roleId}`);
          console.log(response.data); 
          setDeletionMessage(`El rol ${roleName} (${roleId}) ha sido eliminado correctamente.`);
        ...
        <ul>
          {roles.map((role) => (
            <li key={role._id}>
                ...
                <button onClick={()=>setShowDeletionModalFor(role._id)}>Eliminar rol</button>
                ...
                <DeletionModal
                    ...
                    onDelete={() => handleDelete({roleName: role.name,roleId: role._id })}
